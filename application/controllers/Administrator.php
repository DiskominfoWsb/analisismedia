...
class Administrator extends CI_Controller {
	...
	function index(){
		...
		if (isset($_POST['submit'])){
            ...
            if (!empty($recaptcha)) {
    		    ...
    		    $total = $cek->num_rows();
    			if ($total > 0){
                    $x['tag']=$this->model_grafik->get_tag(); ////////////////////
    				$this->session->set_userdata('upload_image_file_manager',true);
    				$this->session->set_userdata(array('username'=>$row['username'],
    								   'level'=>$row['level'],
                                       'id_session'=>$row['id_session']));
                    // $this->load->view('administrator/home',$x)
    				redirect($this->uri->segment(1).'/home',$x);
    			}else{
	...
	function home(){
        $this->load->model('model_grafik');
        $data['tag']=$this->model_grafik->get_tag();
        $data['sentimen']=$this->model_grafik->get_sentimen();
        $data['media_teratas']=$this->model_grafik->get_media_teratas();
        // var_dump($data['tag']);
        // die;
        if ($this->session->level=='admin'){
         
		  $this->template->load('administrator/template','administrator/include/view_home_admin',$data);
        }else{
    ...
    function home_grafik(){
        $this->load->model('model_grafik');
        $data['tag']=$this->model_grafik->get_tag();
        $data['sentimen']=$this->model_grafik->get_sentimen();    
        $data['media']=$this->model_grafik->get_media();    
        $data['media_teratas']=$this->model_grafik->get_media_teratas();

        $data['sentimen_netral_perbulan']=$this->model_grafik->get_sentimen_netral_perbulan();
        $data['sentimen_positif_perbulan']=$this->model_grafik->get_sentimen_positif_perbulan(); 
        $data['sentimen_negatif_perbulan']=$this->model_grafik->get_sentimen_negatif_perbulan();   
        //   var_dump($data);
        // die;
        if ($this->session->level=='admin'){
         
          $this->template->load('administrator/template','administrator/include/view_home_grafik',$data);
        }else{
          $data['users'] = $this->model_app->view_where('users',array('username'=>$this->session->username))->row_array();
          $data['modul'] = $this->model_app->view_join_one('users','users_modul','id_session','id_umod','DESC');
          $this->template->load('administrator/template','administrator/include/view_home_grafik',$data);
    ...
    function print_custom(){ //cari data between
        $this->load->model('model_grafik');
        // $id=$this->session->userdata('id');
		$dari=$this->input->post('x_dari_tanggal');
		$sampai=$this->input->post('x_sampai_tanggal');
        $data['dari']= tgl_indo($dari);
        $data['sampai']= tgl_indo($sampai);
        // var_dump($dari); echo "<br>";
        // var_dump($sampai);
        // die;

		$fields  = "
        SUM(case when tag like '%Wonosobo-Sehat%' then 1 else 0 end) AS Wonosobo_Sehat,
        ...
        $fields3  = "
        SUM(case when id_media = '1' then 1 else 0 end) AS kompascom,
		...
		$data['tag']=$this->model_app->view_where_fields($fields, $where, $table)->result();
		$data['sentimen']=$this->model_app->view_where_fields($fields2, $where, $table)->result();
		$data['media']=$this->model_app->view_where_fields($fields3, $where, $table)->result();
		$data['media_teratas']=$this->db->query("SELECT 
        count(nama_media) as jumlah_media, 
        nama_media as nama_media,
        SUM(case when sentimen like 'netral' then 1 else 0 end) AS netral,
        SUM(case when sentimen like 'positif' then 1 else 0 end) AS positif ,
        SUM(case when sentimen like 'negatif' then 1 else 0 end) AS negatif 
        FROM berita
		...
		$data['jumlah_data']=$this->model_app->view_where($table,$where)->num_rows();
        ...
        if ($this->session->level=='admin'){
            ...
            $this->template->load('administrator/template','administrator/include/view_home_grafik_custom',$data);
    ...
    function print_custom_pdf(){ //cari data between
        $this->load->model('model_grafik');
        // $id=$this->session->userdata('id');
		$dari=$this->input->post('x_dari_tanggal');
		$sampai=$this->input->post('x_sampai_tanggal');
        ...
        if ($this->session->level=='admin'){
            ...
            $this->load->view('administrator/include/view_home_grafik_custom_pdf',$data);
            ...
            // $this->dompdf->render();
            // $this->dompdf->stream("analasisi_media_" . date('d_m_y') . ".pdf");
            ...
            // $this->dompdf->set_paper(array(0,0,$width,$height));
            // $this->dompdf->render();
            // $this->dompdf->stream("analisis_media_" . date('d_m_y') . ".pdf");
          }
	...
	function tambah_listberita(){
		...
		if (isset($_POST['submit'])){
            ...
            $idmedia = $this->db->escape_str($this->input->post('x_media'));
            
            //---
            $named=$this->model_app->view_where('media',array('id_media'=>$idmedia));
            $dk=$named->row_array();
            $nama_media=$dk['nama_media']; //---
            ...
            if ($hasil['file_name']==''){
                    $data = array('id_kategori'=>$this->db->escape_str($this->input->post('a')),
                                    ...
                                    'id_media'=>$this->db->escape_str($this->input->post('x_media')),
                                    'nama_media'=>$nama_media,
                                    'sentimen'=>$this->db->escape_str($this->input->post('x_sentimen')),
            ...
            $data['media'] = $this->model_app->view_ordering('media','id_media','ASC');
	...
	function edit_listberita(){
		...
		if (isset($_POST['submit'])){
            ...
            $media = $this->model_app->view_ordering('media','id_media','DESC');
            $tone = $this->model_app->view_ordering('tone','id','ASC'); 
            if ($this->session->level=='admin'){
			...
			$data = array('rows' => $proses,'tag' => $tag,'record' => $record,'media' => $media, 'tone'=>$tone);
    ...
    function kategorimedia(){
		cek_session_akses('kategorimedia',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['media'] = $this->model_app->view_ordering('media','id_media','DESC');
        }else{
            $data['media'] = $this->model_app->view_where_ordering('media',array('username'=>$this->session->username),'id_media','DESC');
        }
		$this->template->load('administrator/template','administrator/mod_media/view_media',$data);
	}
	...
	function tambah_kategorimedia(){
		...
		if (isset($_POST['submit'])){
            ...
            $hasil=$this->upload->data();
            if ($hasil['file_name']==''){
    			$data = array('nama_media'=>$this->db->escape_str($this->input->post('x_nama_media')));
            }else{
                $data = array('nama_media'=>$this->db->escape_str($this->input->post('x_nama_media')));
            }
			$this->model_app->insert('media',$data);
			redirect($this->uri->segment(1).'/kategorimedia');
		}else{
			$this->template->load('administrator/template','administrator/mod_media/view_media_tambah');
		}
	}
    ...
    function edit_kategorimedia(){
		...
		if (isset($_POST['submit'])){
			...
			$where = array('id_media' => $this->input->post('x_id_media'));
			$this->model_app->update('media', $data, $where);
			redirect($this->uri->segment(1).'/kategorimedia');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('media', array('id_media' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('media', array('id_media' => $id, 'username' => $this->session->username))->row_array();
            }
			$data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_media/view_media_edit',$data);
		}
	}
    ...
    function grafik(){
        $this->load->model('model_grafik');
        $x['tag']=$this->model_grafik->get_data_stok();
        // var_dump($x['tag']);
        // die;
